...
static void tcp_server_task(void *pvParameters)
{
    ...
    while (1)
    {
        ...
        // SO_RCVTIMEO is not supported in ESP-IDF, so we use poll() or select() to set timeout for recv()
        // setsockopt(sock, SOL_SOCKET, SO_RCVTIMEO, &recvTimeout, sizeof(int)); // set receive time out for 5 seconds
        // refer https://pubs.opengroup.org/onlinepubs/000095399/functions/setsockopt.html for more options

        // Convert ip address to string
#ifdef CONFIG_EXAMPLE_IPV4
        if (source_addr.ss_family == PF_INET)
        {
            inet_ntoa_r(((struct sockaddr_in *)&source_addr)->sin_addr, addr_str, sizeof(addr_str) - 1); // Convert an Internet address into a string
        }
#endif
#ifdef CONFIG_EXAMPLE_IPV6
